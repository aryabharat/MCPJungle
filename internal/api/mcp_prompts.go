...
func listPromptsHandler(mcpService *mcp.MCPService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if server == "" {
			...
			prompts, err = mcpService.ListPrompts()
		} else {
			// server specified, list prompts for that server
			prompts, err = mcpService.ListPromptsByServer(server)
		}
		if err != nil {
...
func getPromptHandler(mcpService *mcp.MCPService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		prompt, err := mcpService.GetPrompt(name)
		if err != nil {
...
func getPromptWithArgsHandler(mcpService *mcp.MCPService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		resp, err := mcpService.GetPromptWithArgs(c, request.Name, args)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to get prompt: " + err.Error()})
			return
		}
...
func enablePromptsHandler(mcpService *mcp.MCPService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		enabledPrompts, err := mcpService.EnablePrompts(entity)
		if err != nil {
...
func disablePromptsHandler(mcpService *mcp.MCPService) gin.HandlerFunc {
	return func(c *gin.Context) {
		entity := c.Query("entity")
		if entity == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "missing 'entity' query parameter"})
		...
		disabledPrompts, err := mcpService.DisablePrompts(entity)
		if err != nil {
